...
valid = False
with zipfile.ZipFile('GemRB.zip', 'w', compression=zipfile.ZIP_DEFLATED, compresslevel=9) as zf:
    zf.write('GemRB.sh')

    for src, dest in build_files(Path('gemrb'), 'gemrb'):
        print(src, dest)
        zf.write(str(src), str(dest))

    for src, dest in build_files(Path('build/'), 'gemrb'):
        print(src, dest)
        zf.write(str(src), str(dest))
